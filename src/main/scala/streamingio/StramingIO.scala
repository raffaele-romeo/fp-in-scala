...
object Process {
  ...
  def processFileToFIle[A, B](f: java.io.File,
                              p: Process[String, A],
                              outputPath: String)(g: (Writer, A) => Writer): IO[Writer] = IO {
    @annotation.tailrec
    def go(ss: Iterator[String], cur: Process[String, A], acc: Writer): Writer =
      cur match {
        case Halt() => acc
        case Await(recv) =>
          val next = if (ss.hasNext) recv(Some(ss.next))
    ...
    val s = io.Source.fromFile(f)
    val fileOut = new FileWriter(outputPath)
    try {
      go(s.getLines, p, fileOut.append(""))
...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    val p2: Process[Int, Int] = lift((x: Int) => x * 10)
    //    p2(Stream(1, 2, 3)).toList.foreach(println)

    val transactions = new File(
      getClass.getClassLoader.getResource("transactions.csv").getFile
    )
    println(processFile(transactions, count |> exists(_ > 1000), false)(_ || _).unsafeRunSync())

    val fars = new File(
      getClass.getClassLoader.getResource("fahrenheit.csv").getFile
    )

    println(processFile(fars, convertFahrenheit, List[String]())(_ :+ _).unsafeRunSync())
    processFileToFIle(fars, convertFahrenheit, "output.csv")((x, y) => x.append(y + "\n")).unsafeRunSync()
